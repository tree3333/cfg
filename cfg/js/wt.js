...
body = body.replace(/"vip_show_expire_time"\s*:\s*\d+/g,'"vip_show_expire_time":2130534427')
body = body.replace(/"vip_expire_time"\s*:\s*\d+/g,'"vip_expire_time":2130534427')
body = body.replace(/"user_id"\s*:\s*"\w+/g,'"user_id":"77666677')
body = body.replace(/"nick"\s*:\s*"\w+/g,'"nick":"6Yves')
body = body.replace(/"is_apple_receipt_user"\s*:\s*\w+/g,'"is_apple_receipt_user":true')
body = body.replace(/"vip_current_type"\s*:\s*\d+/g,'"vip_current_type":1')
//
body = body.replace(/"vip_trial_start_time"\s*:\s*\d+/g,'"vip_trial_start_time":1688250476')
body = body.replace(/"vip_trial_day"\s*:\s*\d+/g,'"vip_trial_day":671932800')
body = body.replace(/"vip_current_type"\s*:\s*\d+/g,'"vip_current_type":2')
body = body.replace(/"vip_current_is_trial"\s*:\s*\w+/g,'"vip_current_is_trial":true')
